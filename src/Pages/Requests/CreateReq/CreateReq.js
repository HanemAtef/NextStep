...
const CreateReq = () => {
  ...
  const { formData, applicationTypes, conditions, loading, error } =
  ...
  const validateForm = () => {
    ...
    if (!formData.studentName) {
      ...
      const namePartsCount = formData.studentName
        .trim()
        .split(/\s+/)
    ...
    if (
      conditions.length > 0 &&
      !conditions.some((condition) => condition.checked)
    ) {
  ...
  const handleSubmit = async (e) => {
    ...
    const hasSelectedCondition = conditions.some(
    ...
    try {
      ...
      const result = await dispatch(
        submitApplication(formDataToSubmit)
      ).unwrap();
        ...
        <form className={styles.form} onSubmit={handleSubmit}>
          <div className={styles.formRow}>
            <div className={styles.formGroup}>
              <label htmlFor="applicationType" className={styles.formLabel}>
                نوع الطلب *
              </label>
              <select
                id="applicationType"
          ...
          <div className={styles.formRow}>
            <div className={styles.formGroup}>
              ...
              />
              {studentIdError && (
                <div className={styles.errorText}>{studentIdError}</div>
              )}
              {formErrors.studentId && !studentIdError && (
                <div className={styles.errorText}>{formErrors.studentId}</div>
              )}
            </div>
            <div className={styles.formGroup}>
              ...
              <input
                ...
                onChange={handleChange}
                disabled={loading}
                placeholder="أدخل الاسم الرباعي بالكامل"
              />
              {formErrors.studentName && (
                <div className={styles.errorText}>{formErrors.studentName}</div>
              )}
            </div>
          </div>
          <div className={styles.formRow}>
